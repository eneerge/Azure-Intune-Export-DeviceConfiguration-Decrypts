...
$ExportPath
    ...
    try {
        ...
        elseif(!(Test-Path $ExportPath)){
            ...
            $JSON1 | Set-Content -Encoding Unicode -LiteralPath "$ExportPath\$FileName_JSON"
...
$global:DCPs = Get-DeviceConfigurationPolicy | Where-Object { ($_.'@odata.type' -ne "#microsoft.graph.iosUpdateConfiguration") -and ($_.'@odata.type' -ne "#microsoft.graph.windowsUpdateForBusinessConfiguration") }
$global:DCPs = Get-DecryptedDeviceConfigurationPolicy -dcp $DCPs
foreach($DCP in $global:DCPs){
